...
/**
 * True when the URL is relative (begins with /, # or ?).
 */
export const isRelativeUrl = (url: string): boolean =>
  url.startsWith("/") || url.startsWith("#") || url.startsWith("?");

/**
 * Checks if the supplied string is a valid absolute URL **or** a relative one.
 * Falls back to `isRelativeUrl` when `new URL()` throws.
 */
...
/**
 * Returns a Sanity `StringOptions` object with a **radio** layout from an
 * array of plain strings or `{ title, value }` objects.
 */
...
/**
 * Converts Portable‑Text blocks into a plain string.  If `maxWords` is given,
 * the resulting text is truncated to that number of words and suffixed with an
 * ellipsis ( … ). Useful for generating previews or meta‑descriptions.
 */
...
/**
 * Exponential back‑off retry wrapper for async operations.
 */
 ...
 */
export const findTreeByPath = (root: Tree, path?: string): Tree => {
 ...
 */
export const formatPath = (path: string | undefined | null): string => {
 ...
 */
export const getPathVariations = (path: string | undefined): string[] => {
  if (typeof path !== "string") return [];
