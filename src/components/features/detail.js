...
const Detail = ({
  ...
  return (
    <DetailDiv className={`flex flex-row flex-wrap justify-between my-12`}>
      <Title
        text={company}
        className={`${isMobile ? "flex-basis-3" : "flex-basis-1"}`}
      />
      <SubDetail
        text={employmentDate}
        className={`${
          isMobile ? "flex-basis-3 justify-start" : "flex-basis-1 justify-end"
        } flex mb-3 regular-green`}
      />
      <SubDetail
        text={position}
        className={`flex-basis-3 flex justify-start mb-5 light-gray`}
      />
